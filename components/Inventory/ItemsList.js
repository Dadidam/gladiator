...
import { Tooltip, Button, Card, Icon } from 'antd';
...
class ItemsList extends React.Component {
  ...
  render() {
    const filteredList = this.filterHeroInventory(this.props.hero, this.props.type);
    ...
    const result = filteredList.map((item, i) => {
      ...
      return (
        <Card.Grid style={{ width: '25%', textAlign: 'center' }} key={i}>
          ...
          {this.props.hero.equipment[this.props.type] !== item.id ? (
            <Button.Group>
              <Button
                type="dashed"
                onClick={() => this.props.useItem(item, this.props.hero)}
              >
                <Icon type="skin" /> Equip
              </Button>
              <Button
                type="dashed"
                onClick={() => this.props.sellItem(item, this.props.hero)}
              >
                <Icon type="wallet" /> Sell: +{item.price.sell} coin(s)
              </Button>
              <Button
                type="dashed"
                onClick={() => this.props.deleteItem(item, this.props.hero)}
              >
            ...
            <Button
              type="dashed"
              onClick={() => this.props.takeOffItem(item, this.props.hero)}
            >
